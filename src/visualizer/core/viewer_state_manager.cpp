 ...
 namespace gs::visualizer {
    ...
    void ViewerStateManager::setupEventHandlers() {
        ...
        state::TrainingPaused::when([this](const auto& event) {
            // 暂停计时器
            training_info_->pauseTiming();
        });

        // ==========================================================================
        // 训练恢复事件处理器（新增）
        // ==========================================================================
        state::TrainingResumed::when([this](const auto& event) {
            // 恢复计时器
            training_info_->resumeTiming();
        });

        // ==========================================================================
        // 训练停止事件处理器（新增）
        // ==========================================================================
        state::TrainingStopped::when([this](const auto& event) {
            // 停止计时器
            training_info_->stopTiming();
        });
    }
